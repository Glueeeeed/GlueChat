...
export function FriendsList({ onSelectFriend, selectedFriendId }: FriendsListProps): React.JSX.Element {
  ...
  return (
    <div className="flex flex-col h-full w-80 bg-gray-900/40 border-r border-white/5 backdrop-blur-sm">
      <div className="p-4 space-y-4">
        <h2 className="text-xl font-bold tracking-tight text-white uppercase">Friends</h2>

        <div className="relative group">
          <FaSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-blue-400 transition-colors" size={14} />
          <input
            type="text"
            placeholder="Search friends..."
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="w-full bg-gray-950/50 border border-white/5 text-sm text-white pl-10 pr-4 py-2 rounded-xl focus:ring-2 focus:ring-blue-500/30 outline-none transition-all placeholder-gray-600 shadow-inner"
          />
        </div>
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`px-3 py-1 text-xs font-bold rounded-lg transition-all ${
              filter === 'all' ? 'bg-blue-600 text-white shadow-lg' : 'text-gray-400 hover:text-gray-200 hover:bg-white/5'
            }`}
          >
            ALL
          </button>
          <button
            ...
            className={`px-3 py-1 text-xs font-bold rounded-lg transition-all ${
              filter === 'online' ? 'bg-blue-600 text-white shadow-lg' : 'text-gray-400 hover:text-gray-200 hover:bg-white/5'
            }`}
          >
            ONLINE
          </button>
      ...
      <div className="flex-1 overflow-y-auto px-2 pb-4 scrollbar-thin scrollbar-thumb-white/10 hover:scrollbar-thumb-white/20">
        {filteredFriends.length > 0 ? (
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 p-3 rounded-xl transition-all duration-200 group mb-1 ${
                selectedFriendId === friend.id
                  ? 'bg-blue-600/10 border border-blue-500/20 shadow-[0_0_15px_-5px_rgba(59,130,246,0.3)]'
          ...
          <div className="flex flex-col items-center justify-center h-40 opacity-30">
            <p className="text-xs uppercase tracking-[0.2em] font-bold">No friends found</p>
          </div>
        )}
      </div>
    </div>
